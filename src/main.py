...
import numpy as np
import matplotlib.pyplot as plt # 🔥 BỔ SUNG: Thư viện vẽ ảnh
...
class CityFlowTestTrackDataset(Dataset):
    ...
    # BỔ SUNG: Hàm tính Trajectory Features (Giống hệt tập Train)
    ...
    def __getitem__(self, index):
        ...
        crop_segments = []
        frame_segments = []
        bbox_segments = [] # Chứa trajectory features
        ...
        for idxs in all_segment_indices:
            ...
            for i, f_idx in enumerate(idxs):
                ...
                raw_box_list.append(raw_box) # Thêm box vào list
            ...
            # Tính Trajectory cho từng segment
        ...
        return {
            ...
            "bbox_features": torch.stack(bbox_segments), # Trả về Data Trajectory
            "track_idx": index
        }
...
# ==========================================
# 3. HÀM TRỰC QUAN HÓA TOP 5
# ==========================================
def save_top5_visualization(query_uuid, queries_data, baseline_rank, test_dataset, save_path="top5_example.png"):
    """Hàm trích xuất và lưu ảnh top 5 crop sát xe cho 1 truy vấn"""
    # Lấy câu truy vấn ngôn ngữ tự nhiên
    nl_query = queries_data[query_uuid].get("nl", ["No text"])[0]

    # Lấy danh sách 5 track UUID có điểm cao nhất
    top5_uuids = baseline_rank[query_uuid][:5]

    fig, axes = plt.subplots(1, 5, figsize=(20, 5))
    fig.suptitle(f"Query: {nl_query}\n(Query UUID: {query_uuid})", fontsize=16, wrap=True)

    for i, t_uuid in enumerate(top5_uuids):
        # Tìm index của track trong dataset
        track_idx = test_dataset.list_of_uuids.index(t_uuid)
        track = test_dataset.list_of_tracks[track_idx]

        # Lấy frame đầu tiên và bounding box đầu tiên của track đó
        first_frame_path = os.path.join(test_dataset.dataset_dir, track["frames"][0])
        raw_box = track["boxes"][0] # Cấu trúc: [x, y, width, height]

        # Load ảnh
        img = Image.open(first_frame_path).convert('RGB')
        
        # ĐÃ SỬA: Cắt sát đúng kích thước bounding box, không mở rộng
        box_tight = (
            int(raw_box[0]),                  # x_min
            int(raw_box[1]),                  # y_min
            int(raw_box[0] + raw_box[2]),     # x_max (x + width)
            int(raw_box[1] + raw_box[3])      # y_max (y + height)
        )

        crop = img.crop(box_tight)
    ...
    plt.close()
    print(f"Đã lưu ảnh ví dụ Top 5 (Crop sát) tại: {save_path}")
...
def main():
    ...
    # LƯU Ý: Giữ đúng tham số bạn dùng lúc Train (d_model=256, num_classes=2155)
    ...
    # =======================================================
    # BƯỚC 1: TRÍCH XUẤT ĐẶC TRƯNG VĂN BẢN (Giữ nguyên)
    # =======================================================
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Extracting Tracks"):
            ...
            bbox_features = batch["bbox_features"].to(device) # LẤY TỪ BATCH
            ...
            bbox_flat = bbox_features.view(B * S, T, 18) # Trải phẳng trajectory theo segment
            
            # TRUYỀN BBOX VÀO MODEL
    ...
    if len(query_uuids_list) > 0:
        sample_query_uuid = query_uuids_list[1] # Lấy query thứ hai làm mẫu
        viz_save_path = "./data/json/retrieval/top5_example.png"
        save_top5_visualization(
            query_uuid=sample_query_uuid,
    ...
    print(f"Recall@10 : {recall_10_base:.4f}  ({(recall_10_base*100):.2f}%)") # IN RECALL@10
    print("="*50)
    ...
    if os.path.exists(final_rank_path):
        ...
        print(f"🔹 Recall@10 : {recall_10_final:.4f}  ({(recall_10_final*100):.2f}%)") # IN RECALL@10
        print("="*50)
    else:
        print(f" Chưa tìm thấy '{final_rank_path}'. Hãy đảm bảo script Rerank đã chạy xong!")
