...
# ==========================================
# 2. DATASET ĐÃ NÂNG CẤP (TEMPORAL ENSEMBLING)
# ==========================================
...
def evaluate_rank_json(rank_json_path, gt_mapping_dict):
    """
    Tính MRR và Recall@5 trực tiếp từ file JSON lưu kết quả rank
    gt_mapping_dict: {query_uuid: ground_truth_track_uuid}
    """
    with open(rank_json_path, 'r', encoding='utf-8') as f:
        rank_data = json.load(f)

    mrr, recall_5 = 0.0, 0.0
    ...
    for query_uuid, ranked_track_uuids in rank_data.items():
        ...
        if gt_track_uuid in ranked_track_uuids:
            ...
            # Nếu GT không có trong list rank (thường không xảy ra vì list chứa toàn bộ track)
            pass 

    return mrr / num_queries, recall_5 / num_queries
...
def main():
    ...
    list_of_track_uuids = test_dataset.list_of_uuids # Lưu lại danh sách UUID xe
    
    # --- 4.2 Khởi tạo Model ---
    print("🧠 Tải trọng số mô hình...")
    model = LiMaVLM(d_model=256, d_text_in=512, num_blocks=4, num_classes=2155) 
    checkpoint = torch.load("./checkpoints/models/limavlm_best.pth", map_location=device)
    model.load_state_dict(checkpoint, strict=False) 
    model.to(device)
    model.eval()

    # =======================================================
    # BƯỚC 1: TRÍCH XUẤT VÀ ENSEMBLE ĐẶC TRƯNG VĂN BẢN
    # =======================================================
    ...
    # 🔥 THÊM BIẾN LƯU LẠI UUID VÀ GROUND TRUTH
    query_uuids_list = []
    ...
    with torch.no_grad():
        for track_idx, (query_uuid, q_data) in enumerate(queries_data.items()):
            ...
            # 🔥 LƯU UUID VÀ MAPPING GROUND TRUTH
            query_uuids_list.append(query_uuid)
            # Theo logic gốc của bạn, GT track_idx chính là track_idx vòng lặp này
    ...
    # =======================================================
    # BƯỚC 2: TRÍCH XUẤT VÀ ENSEMBLE ĐẶC TRƯNG TRACKS (VIDEO)
    # =======================================================
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Extracting Tracks"):
            ...
            outputs = model.encode_image(crop_frames=crops_flat, original_frames=frames_flat)
    ...
    # =======================================================
    # Chấm điểm baseline (trước rerank) để so sánh
    mrr_base, recall_5_base = evaluate_rank_json(baseline_rank_path, query_to_gt_uuid_dict)
    ...
    # LƯU Ý: Ở đây mô phỏng việc bạn chạy lệnh python rerank.py từ terminal.
    print("\n⏳ Vui lòng chạy script rerank.py của bạn. Sau khi có file 'final_rank.json'...")
    
    # Đoạn này sẽ chấm điểm cho file rerank nếu file đã tồn tại
    final_rank_path = './data/json/retrieval/final_rank.json'
    if os.path.exists(final_rank_path):
        mrr_final, recall_5_final = evaluate_rank_json(final_rank_path, query_to_gt_uuid_dict)
        print("\n" + "="*50)
