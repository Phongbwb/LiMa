...
class CityFlowTestTrackDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        crop_segments = []
        frame_segments = []
        bbox_segments = [] # 🔥 MỚI: Chứa trajectory features
...
def main():
    ...
    checkpoint = torch.load("./checkpoints/models/3blockslimavlm_epoch_220.pth", map_location=device)
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Extracting Tracks"):
            ...
            bbox_features = batch["bbox_features"].to(device) # 🔥 BỔ SUNG LẤY TỪ BATCH
